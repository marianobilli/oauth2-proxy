...
lint: validate-go-version
	GO111MODULE=on $(GOLANGCILINT) run --timeout 10m0s

.PHONY: build
build: validate-go-version clean $(BINARY)

$(BINARY):
	CGO_ENABLED=0 $(GO) build -gcflags="all=-N -l" -a -installsuffix cgo -ldflags="-X main.VERSION=${VERSION}" -o $@ github.com/oauth2-proxy/oauth2-proxy/v7
